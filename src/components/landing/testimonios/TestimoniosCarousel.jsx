...
export default function TestimoniosCarousel({ testimonials }) {
  ...
  return (
    <div className="relative">
      <div className="flex justify-center gap-6 transition-all duration-500">
        {visibles.map((testimonial, idx) => (
          <div key={idx} className="bg-tc-black-l border border-gray-700 rounded-md p-6 text-center w-full max-w-md hover:shadow-xl transition-transform transform hover:scale-105 mx-auto">
            <MdIcons.MdFormatQuote className="w-8 h-8 text-tc-yellow mb-3 mx-auto" />
      ...
      <button onClick={prev} className="absolute left-0 top-1/2 -translate-y-1/2 bg-tc-black text-tc-yellow rounded-full p-2 shadow hover:bg-tc-yellow hover:text-tc-black transition-colors"><MdIcons.MdChevronLeft className="w-6 h-6" /></button>
      <button onClick={next} className="absolute right-0 top-1/2 -translate-y-1/2 bg-tc-black text-tc-yellow rounded-full p-2 shadow hover:bg-tc-yellow hover:text-tc-black transition-colors"><MdIcons.MdChevronRight className="w-6 h-6" /></button>
    </div>
  );
} 